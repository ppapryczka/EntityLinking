...
def test_create_graph_for_entity_one_level():
    g: nx.Graph = create_graph_for_entity(EntityId(TEST_ENTITY_1), 0)
    assert list(g.nodes()) == [TEST_ENTITY_1]


def test_create_graph_for_entity_two_levels():
    g: nx.Graph = create_graph_for_entity(EntityId(TEST_ENTITY_2), 1)
    assert TEST_ENTITY_2 in list(g.nodes())
    assert "Q215380" in list(g.nodes())


def test_check_if_target_entity_is_in_graph():
    g: nx.Graph = create_graph_for_entity(EntityId(TEST_ENTITY_3), 1)
